...
- 聚蔟索引(主键索引): innodb 使用聚簇索引，叶子节点上存放了整行的数据. 相对于非聚簇索引，查询效率更高，不用二次回表查询. 但是相对的插入新值等会较慢.
   - innodb 在有主键的时候, 会根据主键创建聚簇索引. 
   - 没有主键的时候会用唯一不为空的索引作为主键, 作为表的聚簇索引. 
   - 若以上均不满足, 则 innodb 会创建一个虚拟的聚簇索引.
...
- B+ 树高: MySQL B+ 树的一个节点大小为一页, MySQL 一页大小默认为 16K 页, 主要是基于性能与存储效率之间的权衡，一页. 非叶子节点存的是 key + 指针, 叶子节点存的是数据行. 对于高度为 3 的 B+ 树, key 为 bigint, 8 byte 指针占 6 byte. 单节点可以存储 16 * 1024/(8+6) = 1170 个索引，按一条记录 1k, 单页 16 条数据, 总计可以存放 1170*1170*16 = 21902400 条数据.
...
- MVCC: 多版本并发控制, 是一种用于数据库管理系统(DBMS)的并发控制方法. 它允许数据的读操作和写操作并行执行而不会互相阻塞，从而提高了数据库的性能和可扩展性。主要通过维护数据的多个版本来实现。MVCC 支持不同的事务隔离级别，如读未提交、读已提交、可重复读和序列化等。
- RU: Read Uncommitted 读未提交是隔离级别最低的一种事务级别，在这种级别下，一个事务会读到另一个事务更新后但未提交的数据。如果一个事务回滚，那么当前事务读到的数据就是脏数据，这就是脏读(Dirty Read)。
- RC: Read Committed, 已称为不可重复读. 大多数数据库系统的默认隔离级别。满足了隔离的简单定义，一个事务只能看见已提交的事务所做的改变。MVCC 在 RC 隔离级别中，每次 select 语句都生成一个 read-view。
- RR: Repeatable Read 可重复读, 是 mysql 的默认隔离级别，它确保一个实例在并发读取数据时，会看到同样的数据行，但这会导致幻读。innodb 通过 MVCC 解决幻读问题。MVCC 在 RR 隔离级别中，第一次执行 select 生成一个 read-view，这个事务中后面所有的 select 语句都复用 read-view。
- S: Serializable 可串行化，是最高级别的隔离级别，它通过强制事务排序，使之不可能相互冲突，从而解决幻读问题。
- undo: 主要用于支持事务的回滚操作以及实现多版本并发控制(MVCC)。事务回滚即撤销事务操作恢复到事务执行前的状态。对 MVCC 的支持则是利用 undo 日志创建数据的早期版本，这允许查询看到特定时间点的数据状态。
- redo: 侧重于确保事务一旦提交后即使发生系统故障也能永久生效，重点在于提升性能和保障数据的持久性。
