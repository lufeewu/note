...
- RC: Read Committed , 已称为不可重复读. 大多数数据库系统的默认隔离级别。满足了隔离的简单定义，一个事务只能看见已提交的事务所做的改变。
- RR: Repeatable Read 可重复读, 是 mysql 的默认隔离级别，它确保一个实例在并发读取数据时，会看到同样的数据行，但这会导致幻读。innodb 通过 MVCC 解决幻读问题。
