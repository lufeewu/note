...
- training loss: 训练损失, 用于衡量模型在训练集上拟合能力的指标. 通过计算训练集中每个样本的预测误差总和或平均值. 模型在训练过程中, 再一次对训练样本进行前向传播计算, 从而衡量当前模型权重下, 预测结果与真实标签之间误差的过程.
- validation loss: 验证损失, 用于衡量模型在未见过的数据(验证集)上的预测误差. 反映了模型的泛化能力, 即模型将训练中学到的知识推广到新数据的能力.
