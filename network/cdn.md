...
调度策略优化: 冷热调度、最短路径策略、带宽负载均衡策略、响应时间策略、内容感知策略等.
+ 最短路径策略: 根据用户位置选择最近 CDN 节点.
+ 带宽负载均衡策略: 根据 CDN 的带宽与负载情况, 动态分配流量, 保证系统的负载均衡和稳定性.
+ 响应时间策略: 根据不同 CDN 节点的响应时间, 选速度最快的节点提供服务.
+ 内容感知策略: 根据内容的类型和特征, 选择最适合该类型内容的 CDN 节点提供服务.
+ 冷热调度: 冷、热内容采取不同的调度策略, 降低冷内容回源率.
