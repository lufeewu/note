...
### 缓存击穿
缓存击穿是指一个热点 Key 在缓存过期的瞬间, 海量高并发请求直接越过缓存层, 同时冲击数据库, 导致数据库段时间内负载骤增甚至宕机. 可以考虑如下解决方案:
- 热点数据永不过期: 通过后台定时异步刷新缓存
- 加锁(互斥锁) 更新: 缓存失效后, 仅允许一个线程/请求查询数据库并重构缓存, 其它请求等待或重试, 防止并发数据库冲击.
- 预热: 秒杀活动前预先将热点数据存入缓存.
- 逻辑过期: redis 中不设置过期时间, 在数据中封装逻辑过期时间. 缓存命中后, 比对数据是否过期, 过期则通过持锁的独立线程异步更新.

### 缓存雪崩
缓存雪崩是同一时刻, 大规模数据同时失效或缓存服务宕机, 导致海量高并发请求直接涌入数据库, 瞬间压垮数据库导致整个系统崩溃的现象. 解决方案:
- 随机过期时间: 在基础过期时间上加上随机过期时间, 分散缓存失效时间, 避免集中过期.
- 缓存高可用: redis 哨兵模式或集群保证缓存系统高可用, 防止节点单点故障.
- 熔断降级: 缓存崩溃后, 采用限流、熔断措施, 暂停向数据库请求.
- 二级缓存: 使用本地缓存作为一级缓存, Redis 作为二级缓存, 多重防护.

### 缓存穿透
缓存穿透是用户查询的数据既不在 Redis 缓存中, 也不在数据库中, 导致每次请求都透过缓存直接查询数据库, 造成数据库压力激增导致宕机. 解决方案:
- 缓存空对象: 查询结果为空的时候, 缓存中也存储空值，设置较短的过期时间, 防止同类请求再次查询.
- 布隆过滤器: 查询缓存前, 使用布隆过滤器判断数据是否存在. 若过滤器判定数据不存在, 则直接返回, 无需查询数据库.
- 参数校验: 应用层对参数进行合理性校验, 过滤非法请求.
